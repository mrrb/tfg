\chapter{Máquinas de estados \emph{Mealy} de los módulos diseñados}
\label{ch:maquinas-estados}
...
\begin{figure}[!p]
    ...
    \begin{tikzpicture}
        ...
        \node[state] at ($ (IDLE)+(0, -4.25) $) (IDLE) {ULPI\_IDLE};
        \node[state] at ($ (IDLE)+(-90:4.75) $)(RECV) {ULPI\_RECV};
        \node[state] at ($ (IDLE)+(-150:4.75) $) (REGW) {ULPI\_REG\_WRITE};
        \node[state] at ($ (IDLE)+(-30:4.75) $) (REGR) {ULPI\_REG\_READ};
...
\begin{figure}[!p]
    ...
    \begin{tikzpicture}
        \tikzFSM
        \node[state, initial, accepting] (IDLE) {SERIAL\_OP\_IDLE};
        \node[state] at ($ (IDLE)+(0, -4.5) $) (READ) {SERIAL\_OP\_READ};
        \node[state] at ($ (READ)+(0, -4) $) (PUSH) {SERIAL\_OP\_PUSH};
    ...
    \end{tikzpicture}
    \caption{Máquina de estados del módulo ULPI\_op}
    \label{fig:mealy_ulpi_op}
\end{figure}
\pagebreak
...
\begin{figure}[!p]
    ...
    \label{fig:mealy_main_controller}
\end{figure}